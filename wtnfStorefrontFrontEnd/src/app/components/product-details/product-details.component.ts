...
export class ProductDetailsComponent implements OnInit {
  ...
  getImageUrls() {
    if (this.product.image_ids) {
      this.image_paths = this.productDetailsService.getProductImageUrls(
    ...
    if (this.product.color_ids) {
      for (let i = 0; i < this.product.color_ids.length; i++) {
        this.productDetailsService
          .getColor(this.product.color_ids[i])
          .subscribe(color => {
            if (color.name) {
  ...
  getMaterialNames() {
    if (this.product.material_ids) {
      for (let i = 0; i < this.product.material_ids.length; i++) {
        this.productDetailsService
          .getMaterial(this.product.material_ids[i])
          .subscribe(material => {
            if (material.name) {
        ...
        this.cartService.getCart().subscribe({
          ...
          next: cartItems => {
            ...
            for (let i = 0; i < cartItems.length; i++) {
              ...
              if (
                this.product.id &&
                cartItems[i].product_id + '' == this.product.id
              ) {
                isProdInCart = true;
                prodIndex = i;
                break;
            ...
            if (isProdInCart) {
              ...
              if (
                this.product.boh &&
                this.product.id &&
                cartItems[prodIndex].product_quantity + this.quantity <=
                  this.product.boh
              ) {
                // update product quantity in cart
                this.cartService
                  .updateProductQuantityInCart(this.product.id, this.quantity)
                  .subscribe({
                    error: err => {
                      alert('Product quantity NOT updated in cart.');
                    },
                    next: cartItem => {
